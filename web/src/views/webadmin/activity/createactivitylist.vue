...
export default {
  ...
  methods: {
    ...
    async del(activity_id) {
      ...
      let res = await this.$API.reqDeleteActivity(
...
//取消参加活动
    async deljoin(activity_id) {
      let res = await this.$API.reqDeleteJoin(
    ...
    async getactivitylist() {
      ...
      let res = await this.$API.reqGetWebActivityList(
...
//获取参与活动列表
    async getactivityjoinlist() {
      let res = await this.$API.reqJoinsList(
      );
      if (res.data.state.type === "SUCCESS") {
