...
export default {
  data() {
    return {
      ...
      form: {
        activity_title: "",
        activity_impose: false, //是否限制人数
        activity_num: 0, //设置参与人数
        activity_type: "", //线上还是线下
        activity_lable: "", //标签
        activity_content: "",
        activity_img: "",
        activity_statetime: "",
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      if (this.id) {
        ...
        res = await this.$API.reqUpdateActivity(
          this.qs.stringify(this.form)
        );
      } else {
        res = await this.$API.reqCreateActivity(
          this.qs.stringify(this.form)
        );
      }
    ...
    async setannouncement() {
      ...
      let res = await this.$API.reqSetAnnouncement(
...
//获取活动详细内容
    async getactivitydetails() {
      const res = await this.$API.reqGetActivityDetail(
...
//获取参与人员列表
    async getjoinslist() {
      const res = await this.$API.reqGetJoinList(
        this.qs.stringify({ id: this.id })
      );
