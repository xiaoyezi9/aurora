        ...
        >
          <article id="comment-2">
            <el-popover placement="top" width="400" trigger="hover">
              <span>
                <span class="comment even thread-odd thread-alt depth-1" id="li-comment-4">
...
export default {
  ...
  methods: {
    jubao(username) {
      ...
      // let url = urlencodeComponet(window.location.href); //escape对url进行编码方便在地址栏传递
      this.$router.push({
        path: "/jubao",
    ...
    async setcomment() {
      ...
      if (this.touserid != "") {
        ...
        console.log(data)
        res = await this.$API.reqSetreply( this.qs.stringify(data));
        ...
        res = await this.$API.reqSetComment(
...
//获取回复
    async getreply() {
      ...
      let res = await this.$API.reqGetReply(
          this.qs.stringify(data)
      );
      if (res.data.state.type === "SUCCESS") {
    ...
    async getcomment() {
      ...
      let res = await this.$API.reqGetComment(
        this.qs.stringify(data)
      );
      if (res.data.state.type === "SUCCESS") {
