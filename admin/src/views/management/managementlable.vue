...
export default {
  ...
  methods: {
    ...
    async change(id) {
      ...
      let res = await this.$API.reqChangeLable(this.qs.stringify(this.changelable));
...
//获取标签列表
    async lablelist() {
      let res = await this.$API.reqLableList(this.qs.stringify({ lable_name: "" }));
      if (res.data.state.type === "SUCCESS") {
