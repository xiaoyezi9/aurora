      ...
      <div class="main">
        <div class="user-info clearfix">
          ...
          <ul class="first-userInfo">
            <li class="user-li">
              账号 :
              <span class="user-name" title="用户名">{{tableData.username}}</span>
            </li>
            <li>
              昵称 :
          ...
          <ul>
            ...
            <li>
              ...
              <span style="margin-left:20px" v-else>管理员</span>
            </li>
            <li>
              权限 :
              <span
              ...
              >反馈中心</span>
              <span style="margin-left:20px" v-if="tableData.username=='admin'">最高权限</span>
            </li>
          </ul>
          <ul>
            <li>
...
export default {
  ...
  methods: {
    ...
    async changeadminuser() {
      let res = await this.$API.reqUpdateUserInfo(this.qs.stringify(this.tableData) );
...
//获取管理员信息--通过token
    async getadmin() {
      let res = await this.$API.reqAdminInfo(this.qs.stringify(this.tableData) );
      if (res.data.state.type==='SUCCESS') {
